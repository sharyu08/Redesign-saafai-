    ...
    return (
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-xl overflow-hidden print:shadow-none print:border-slate-300">
            ...
            <div className="lg:hidden space-y-4 p-4">
                {data.map((item) => (
                    <div key={item.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 space-y-4 hover:bg-[#F4FBFC]/40 transition-colors">
                        ...
                        <div className="flex items-center gap-3 pb-3 border-b border-slate-100">
                            ...
                            <div className="flex-1">
                                <h3 className="text-base font-bold text-slate-800">{item.cleaner}</h3>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {/* Date & Time */}
                            <div className="flex items-center gap-2">
                                <Calendar size={16} className="text-[#58BECF]" />
                                <div>
                            ...
                            {/* Zone */}
                            <div className="flex items-center gap-2">
                                <MapPin size={16} className="text-[#58BECF]" />
                                <div>
                                    <p className="text-sm font-medium text-slate-700">{item.zone}</p>
                            ...
                            {/* Status */}
                            <div className="flex items-center gap-2">
                                <Activity size={16} className="text-[#58BECF]" />
                                <span className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-widest border ${item.status === 'Completed' ? 'bg-emerald-50 border-emerald-100 text-emerald-500' : 'bg-amber-50 border-amber-100 text-amber-500'
                                    }`}>
                                    {item.status}
                                </span>
                            </div>
